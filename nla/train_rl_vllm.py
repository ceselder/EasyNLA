...
def grpo_token_loss(new_lp, ref_lp, advantage, *, kl_beta=0.01, kl_tok=None,
    ...
    elif loss_mode == "ppo" and old_lp is not None and old_lp.numel() >= new_lp.numel():
        # PPO clipped surrogate against the sampler's logprobs: needed when rollouts were generated by weights one step stale (--async-gen).
        ratio = torch.exp(new_lp - old_lp[: new_lp.numel()].to(new_lp.device))
        surrogate = torch.min(ratio * advantage, ratio.clamp(1.0 - ppo_clip, 1.0 + ppo_clip) * advantage)
    else:
...
def grpo_update_microbatched(
    ...
    # length-sorted micro-batches: rollouts of similar length share a micro-batch -> far less padding (pure speed; same math)
    _order = sorted(range(n), key=lambda i: -full_ids_list[i].numel()) if sort_by_length else list(range(n))
...
def main():
    ...
    p.add_argument("--loss", choices=["reinforce", "cispo", "ppo"], default="reinforce",
    ...
    if _async:
        assert flow is not None and args.reward_mode == "flow", "--async-gen supports the flow-reward path only"
        assert not getattr(flow, "use_tokens", False), "--async-gen needs an actor-free conditioner (tokens_ar / tokens_base / ar_vec with its own trunk)"
        ...
        class _BgJob:
            def __init__(self, fn):
                self.result = None; self.exc = None; self.done = False
                self.t = threading.Thread(target=self._run, args=(fn,), daemon=True); self.t.start()
            def _run(self, fn):
                try: self.result = fn()
                except BaseException as e: self.exc = e
            def join(self):
                if self.t.is_alive(): self.t.join()
                if self.exc is not None: raise self.exc
                return self.result

        def _draw_batch():
            nonlocal cursor
            if cursor + args.batch_prompts > len(pending_idxs):
                rng.shuffle(pending_idxs); cursor = 0
            b = pending_idxs[cursor: cursor + args.batch_prompts]; cursor += args.batch_prompts
            if is_dist:
                _lbp = args.batch_prompts // world_size; b = b[rank * _lbp:(rank + 1) * _lbp]
            pa, bs_ = [], []
            for row_idx in b:
                row = rows[row_idx]
                pa.append((build_prompt_text(row["prompt"], inject_char, tok_bg), torch.tensor(row["activation"], dtype=torch.float32))); bs_.append(row.get("source", ""))
            return b, pa, bs_

        def _gen_parse_score(pa, bs_, gen_step, do_critic):
            """Runs on the background thread: rollouts (engine on the critic GPU) -> parse -> flow reward (critic GPU) -> critic backward on gold pairs."""
            R = {"t0": time.time()}
            try: llm.apply_model(read_reset_steer_count)
            except Exception: pass
            responses = rollout_batch_vllm(llm, tok_bg, pa, inj_id, args.group_size, args.max_new_tokens, args.temperature, left_id=left_id, right_id=right_id)
            R["t_gen"] = time.time() - R["t0"]; sc = -1
            try:
                _sc = llm.apply_model(read_reset_steer_count)
                if _sc and all(c >= 0 for c in _sc): sc = min(_sc)
            except Exception: sc = -1
            R["steer_apply_count"] = sc; R["responses"] = responses
            L = {k: [] for k in ("full_ids", "prompt_lens", "activations", "explanations", "response_text", "prompt_group", "old_logps", "truncated", "steer_verified")}
            for r in responses:
                L["full_ids"].append(r["full_ids"]); L["prompt_lens"].append(r["prompt_len"]); L["activations"].append(pa[r["prompt_idx"]][1])
                L["explanations"].append(extract_explanation(r["text"])); L["response_text"].append(r["text"]); L["prompt_group"].append(r["prompt_idx"])
                L["old_logps"].append(r["old_logp"].to(device)); L["truncated"].append(bool(r.get("truncated", False))); L["steer_verified"].append(bool(r.get("steer_verified", True)))
            R["L"] = L; t1 = time.time()
            with critic_ema.swapped():
                fr, rw, rp = flow.score(L["explanations"], L["activations"], L["prompt_group"], seed=gen_step)
            R["flow_rewards"], R["rewards"], R["recon_preds"] = fr, rw, rp; R["t_score"] = time.time() - t1; t2 = time.time()
            R["critic_acc"] = float("nan")
            if do_critic and args.train_critic and critic_optim is not None:
                R["critic_acc"] = flow.train_backward_grounded(max(len(responses), 64), max(1, args.grad_accum), seed=gen_step * 7919 + rank)
            R["t_critic"] = time.time() - t2; R["pa"] = pa; R["bs"] = bs_
            return R
    for step in range(args.start_step, args.num_steps):
        ...
        if not _async:
            ...
            # ---- ASYNC: the background job (launched last step) generated + scored this batch under the engine weights of one
            # step ago; join it, apply the critic gradients it accumulated, then launch the job for the NEXT step before GRPO. ----
            actor.eval()
            _t_top = time.time()
            if pending is None:
                _b, _pa, _bs = _draw_batch(); R = _gen_parse_score(_pa, _bs, step, True)
            else:
                R = pending.join(); pending = None
            bg_wait_s = time.time() - _t_top
            critic_loss_val_async = float(R["critic_acc"]); critic_grad_norm_async = float("nan")
            if args.train_critic and critic_optim is not None:
                if not math.isfinite(R["critic_acc"]):
                    for p_ in critic_trainable:
                        if p_.grad is not None: p_.grad.zero_()
                if is_dist: _allreduce_grads_(critic_trainable, world_size)
                _cg = torch.nn.utils.clip_grad_norm_(critic_trainable, args.max_grad_norm); _cg = _cg.item() if hasattr(_cg, "item") else float(_cg)
                if math.isfinite(_cg):
                    critic_ema.assert_live("(async critic step)"); critic_optim.step(); critic_ema.update()
                critic_optim.zero_grad(set_to_none=True); critic_grad_norm_async = _cg
            _b2, _pa2, _bs2 = _draw_batch()
            pending = _BgJob(lambda: _gen_parse_score(_pa2, _bs2, step + 1, True))
            prompts_with_acts, batch_sources, responses = R["pa"], R["bs"], R["responses"]; L_ = R["L"]
            all_full_ids, all_prompt_lens, all_activations, all_explanations, all_response_text, all_prompt_group, all_old_logps, all_truncated, all_steer_verified = (
                L_["full_ids"], L_["prompt_lens"], L_["activations"], L_["explanations"], L_["response_text"], L_["prompt_group"], L_["old_logps"], L_["truncated"], L_["steer_verified"])
            steer_apply_count = R["steer_apply_count"]; n_rollouts_gen = len(responses)
            steer_apply_rate = (steer_apply_count / n_rollouts_gen if steer_apply_count >= 0 and n_rollouts_gen else float("nan"))
            if 0 <= steer_apply_count < int(0.98 * n_rollouts_gen):
                print(f"step {step}: WARNING vLLM steering applied to only {steer_apply_count}/{n_rollouts_gen} rollouts (<98%)", flush=True)
            cjk_fail = [cjk_fraction(t) > 0.05 for t in all_response_text]
            marker_ok = [marker_well_formed(all_full_ids[i].tolist(), inj_id, left_id, right_id) for i in range(len(all_full_ids))]
            inject_ok = [(not cjk_fail[i]) and marker_ok[i] and all_steer_verified[i] for i in range(len(all_full_ids))]
            n_inject_fail = int(sum(cjk_fail)); n_marker_bad = int(sum(1 for m in marker_ok if not m))
            n_steer_unverified = int(sum(1 for v in all_steer_verified if not v)); n_inject_masked = int(sum(1 for ok in inject_ok if not ok))
            inject_ok_t = torch.tensor(inject_ok, dtype=torch.bool, device=device); n_truncated = int(sum(all_truncated))
            t_gen_end = t0 + R["t_gen"]; t_roll_end = time.time()
            flow_rewards, rewards, recon_preds = R["flow_rewards"], R["rewards"], R["recon_preds"]
            rewards = [None if t else r for r, t in zip(rewards, all_truncated)]
            flow_rewards = [None if t else r for r, t in zip(flow_rewards, all_truncated)]
            print(f"  [async@{step}] bg gen {R['t_gen']:.0f}s score {R['t_score']:.0f}s critic {R['t_critic']:.0f}s | waited {bg_wait_s:.0f}s | next job launched", flush=True)
        ...
        for _attempt in range(5 if args.oom_keep_prefix else 3):
          try:
            ...
            if not isinstance(_oom_e, torch.OutOfMemoryError) and "out of memory" not in str(_oom_e).lower(): raise   # Triton kernels raise OOM as RuntimeError('Triton Error [CUDA]: out of memory')
        ...
        vllm_sync_secs = 0.0
        if _async and pending is not None:   # the engine must be idle before its weights change: wait for the background job (usually already done)
            _tw = time.time(); pending.join(); bg_wait_s += time.time() - _tw
        if is_accum_end:
        ...
        if _async and args.train_critic and critic_optim is not None:
            critic_loss_val = critic_loss_val_async; critic_grad_norm_val = critic_grad_norm_async
        if is_dist and args.train_critic and critic_optim is not None and is_accum_end and _critic_now and not _async:
        ...
        if is_main and not args.no_wandb:
            if _async: log.update({"time/bg_gen_s": R["t_gen"], "time/bg_score_s": R["t_score"], "time/bg_critic_s": R["t_critic"], "time/bg_wait_s": bg_wait_s})
