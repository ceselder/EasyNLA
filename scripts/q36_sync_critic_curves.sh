...
for i in $(seq 1 60); do
  out=$(systemd-run --user --scope -q -p MemoryMax=2G python3 scripts/plot_nlt_q36_critic_train_curves.py --tags v1,v2,v1b,v3b 2>&1 | grep -E "^v1b|^v3b" | tail -n 2 | tr '\n' ';')
...
ok=all((d.get(t,{}).get('evals') or [{}])[-1].get('step',0)>=3000 for t in ('v1b','v3b')); sys.exit(0 if ok else 1)" && { log "CURVES DONE"; break; }
  sleep 600
done
