...
RL_TAG=${RL_TAG:-rl_v4b}; FROZEN=${FROZEN:-/vol/q36/critic/v1b/ckpt_step000500.pt}; OTHER=${OTHER:-/vol/q36/critic/v2/ckpt_step3000.pt}; JUDGE2=${JUDGE2:-/vol/q36/critic/v4/ckpt_step002000.pt}; EVERY=${EVERY:-20}   # orchestrator 16:12: judge2 = critic v4 @2000 (claim-sensitive second judge); other = v2 s3000 = plausibility column only
...
for i in $(seq 1 400); do
  ...
  for d_ in $dumps; do st=${d_#dumps_}; st=${st%.parquet}; stn=$((10#$st)); [ $((stn % EVERY)) -eq 0 ] || continue
    for J in frozen other judge2; do
      out=rl_${RL_TAG}_exact_${J}_$st; [ -n "${Q[$out]:-}" ] && continue; [ -f $D/$out.json ] && { Q[$out]=1; continue; }
      grep -q "^\[$out\] QUEUED" $LOGD/evalq.txt 2>/dev/null && { Q[$out]=1; continue; }
      CK=$FROZEN; [ $J = other ] && CK=$OTHER; [ $J = judge2 ] && CK=$JUDGE2
  ...
  new=0
  for f in $(timeout 120 modal volume ls nlt q36/results 2>/dev/null | grep -oE "rl_${RL_TAG}_exact_(frozen|other|judge2)_[0-9]+\.json" | sort -u); do
