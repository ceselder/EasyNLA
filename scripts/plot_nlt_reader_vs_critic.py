...
def main():
    ...
    style(); plt.rcParams.update({"xtick.labelsize": 13, "ytick.labelsize": 13, "axes.labelsize": 13, "axes.titlesize": 14, "legend.fontsize": 12})
    x = np.arange(len(rows)); w = 0.38
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 6.6), dpi=150)
    ...
    fig.suptitle("\n".join(textwrap.wrap("Readers recover the next token 3× above chance from the verbalizer's sentence, but the flow critic pays ~1 bit for it and ranks the sources in the opposite order", 78)), fontsize=14, x=0.01, y=0.995, ha="left", va="top")
    ...
    fig.subplots_adjust(left=0.085, right=0.985, top=0.86, bottom=0.27, wspace=0.30)
