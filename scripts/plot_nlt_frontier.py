...
def main():
    ...
    style(); fig, ax = plt.subplots(figsize=(11, 7.6), dpi=150); out = {"points": [], "reference": None, "guides_bits_per_token": [0.01, 0.1, 1.0]}
    xs_all, ys_all = [], []
    for stem, (lab, col, mk, c) in by_ckpt.items():
        ...
        for p in pts:
            if p[3] in ("lens_L0", "lens_L1", "lens_L3", "teacher_v1", "v0", "teacher_v0", "ao_tgt"): ax.annotate(SET_SHORT.get(p[3], p[3]), (p[0], p[1]), textcoords="offset points", xytext=(6, 4), fontsize=9, color=col)
    if t2:
    ...
    for bpt in out["guides_bits_per_token"]:
        ax.plot(xr, bpt * xr, color=GRID, lw=1.2, ls=(0, (4, 3)), zorder=1); ax.text(200, bpt * 200 * 1.08, f"{bpt:g} bits / token", color=INK2, fontsize=9.5, ha="right", va="bottom")
    ax.set_xscale("log"); ax.set_yscale("symlog", linthresh=1); ax.set_xlim(5, 220); ax.set_ylim(-0.5, 25)
    ...
    ax.set_title("Natural-language sources sit between 0.01 and 0.2 bits per token and gain sub-linearly with length;\nthe raw lens list at the same length is 4–6× higher", loc="left", fontsize=12.5)
    fig.suptitle("\n".join(textwrap.wrap("The bits-per-token frontier the spec asks for is concave and low: a 9-token phrase buys ~1.7 content bits, a 40-token sentence ~2, a 137-token list ~3 "
                                          "(0.18 → 0.05 → 0.02 bits/token) under the pooled critics — PRELIMINARY, every critic below its gate", 100)), fontsize=13.5, x=0.01, y=0.995, ha="left", va="top")
