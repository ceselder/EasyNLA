...
systemd-run --user --scope -q -p MemoryMax=1G --setenv=RL_TAG=rl_v5 --setenv=CRITIC_CK=/vol/q36/critic/v5/ckpt_step000849.pt --setenv=FROZEN_CK=/vol/q36/critic/v5/ckpt_step000849.pt --setenv=GATE_DIR=q36/critic/v5 --setenv=PRIO=1 bash $LOGD/launch_rl_v5.sh >> $LOGD/launch_rl_v5.out 2>&1 &
...
systemd-run --user --scope -q -p MemoryMax=1G --setenv=RL_TAG=rl_v5 --setenv=FROZEN=/vol/q36/critic/v5/ckpt_step000849.pt --setenv=APPFILE=$LOGD/rl_app_rl_v5.txt bash $LOGD/rl_exact_evals.sh >> $LOGD/rl_exact_evals_v5.out 2>&1 &
