...
for i in $(seq 1 600); do [ "$(nfiles q36/critic/v1b 'ckpt_final.pt')" -ge 1 ] && break; sleep 180; done; log "critic v1b final exists"
timeout 120 modal volume get nlt q36/critic/v1b/best.json /tmp/q36_v1b_best.json --force >/dev/null 2>&1; log "v1b best.json: $(cat /tmp/q36_v1b_best.json 2>/dev/null)"
for C in final best; do
  OUT=bits_v1b_main; [ $C = best ] && OUT=bits_v1bbest_main
  wait_gpu 1 || exit 1
...
wait_bits bits_v1b_main bits_v1bbest_main || exit 1; for f in bits_v1b_main bits_v1bbest_main; do cp /tmp/q36_chk_$f.json $D/$f.json; done
timeout 120 modal volume get nlt q36/critic/v1b/eval_latest.json $D/critic_v1b_eval_latest.json --force >/dev/null 2>&1
for f in bits_v1b_main bits_v1bbest_main; do python3 -c "
