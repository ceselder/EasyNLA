...
# ready = the file is listed AND its size is unchanged across two listings 90 s apart (a chain once fired on a half-written checkpoint)
size() { modal volume ls nlt "$PRIOR_DIR" 2>/dev/null | grep "$(basename "$PRIOR")" | tr -d '│' | awk '{print $3, $4}'; }
until [ -n "$(size)" ]; do sleep 60; done
S1=$(size); sleep 90; S2=$(size)
until [ "$S1" = "$S2" ]; do S1=$S2; sleep 90; S2=$(size); done
echo "[$(date -u +%H:%M:%S)] prior present; launching arms ${ARMS:-e0 e1 e2 e3}" | tee -a $LOG/enc_ablation.log
for arm in ${ARMS:-e0 e1 e2 e3}; do
  setsid nohup bash scripts/nlt_lens_enc_arm2.sh $arm ${STEPS:-4000} > $LOG/enc_arm_$arm.out 2>&1 < /dev/null &
  sleep 5
done
wait
