...
def main():
    ...
    ax.axhline(0.5, color="k", lw=0.8); ax.axhline(0.65, color=CG, ls="--", lw=1); ax.set_ylim(0.3, 1.0); ax.set_xlabel("RL step"); ax.set_ylabel("P(true crafted text > one-claim twin)"); ax.set_title("Does RL move claim sensitivity? P(true > one-swapped-claim twin), FROZEN judge", fontsize=12); ax.legend(frameon=False, fontsize=9)
    ax = axes[1, 0]
    ...
    ax = axes[1, 1]; ax.plot(steps, curves["cotrained_rp"], "o-", color=C1, lw=2, label="co-trained: random-pair text bits"); ax.plot(steps, curves["frozen_rp"], "s-", color=C2, lw=2, label="frozen: random-pair text bits"); ax.axhline(0, color="k", lw=0.8)
    ...
    ax.plot(steps, curves["tokens"], "o-", color=C2, lw=2, label="tokens per readout (held-out greedy)"); ax.axhline(0, color="k", lw=0.8); ax.set_xlabel("RL step"); ax.set_ylabel("tokens"); ax.set_title("No length collapse (rl_v1 fell 96 -> 1 in 7 steps)?", fontsize=13); ax.legend(frameon=False, fontsize=9)
    ax = axes[2, 1]; ax.plot(steps, curves["tokens"], "o-", color=C4, lw=2, label="tokens per readout (held-out greedy)"); ax2 = ax.twinx(); ax2.plot(steps, np.array(curves["depth_hit_rate"]) * 100, "^--", color=C3, lw=1.5, label="depth-word hits (%)"); ax2.set_ylabel("% readouts with a depth word")
