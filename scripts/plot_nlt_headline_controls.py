...
VARIANTS = [("orig", "the pair's own sentence z", CAT[0]), ("dm", "another pair's sentence at the same (i, j) — the depth-matched wrong sentence", CAT[1]), ("rp", "a random pair's sentence", CAT[2]), ("shuf_words", "own sentence, words permuted", "#b3b1a8")]
...
def main():
    ...
    verdict = (f"pays each source about a bit of pair-specific content (P(own beats the depth-matched wrong sentence) {pmin:.2f}–{pmax:.2f}, none over the 0.75 gate)" if n_pass == 0 else
