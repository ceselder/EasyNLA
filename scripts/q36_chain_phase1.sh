...
CRITIC_STEPS=${CRITIC_STEPS:-3000}; SFT_STEPS=${SFT_STEPS:-600}; SKIP_TO=${SKIP_TO:-}
...
if [ "$(nfiles q36/critic/$TAG 'ckpt_final.pt')" -lt 1 ]; then
  run hf 1 train_critic.py "--data-dir /vol/q36/data --out /vol/q36/critic/$TAG --tag critic_$TAG --pools '$POOLS' --val-sets '$VALS' --band $BAND --width 1536 --depth 16 --heads 16 --param v --steps $CRITIC_STEPS --batch 1024 --micro-batch 128 --eval-every 500 --eval-n 256 --spot-exact-n 64 --spot-ode-steps 16 --max-hours 3.0" critic
fi
if [ "$(nfiles q36/verbalizer/$TAG/final 'adapter_model')" -lt 1 ]; then
  run hf 4 sft_verbalizer.py "--data-dir /vol/q36/data --text '$TX/train/craft_full__*.parquet' --val-text '$TX/val/craft_full__*.parquet' --out /vol/q36/verbalizer/$TAG --band $BAND --steps $SFT_STEPS --batch 8 --grad-accum 4 --lr 3e-5 --eval-every 100 --save-every 200 --wandb-name sft_$TAG" sft 4
fi
