...
CRITIC_STEPS=${CRITIC_STEPS:-3000}; ENGINES=${ENGINES:-4}
...
if [ ! -f "$MARK" ]; then
  ...
  run vllm-many 1 rollout_vllm.py "${ARGS% ;; }" rollouts_w
fi
...
if [ "$(nfiles q36/critic/$TAG 'ckpt_final.pt')" -lt 1 ]; then
  run hf 1 train_critic.py "--data-dir /vol/q36/data --out /vol/q36/critic/$TAG --tag critic_$TAG --pools '$POOLS' --val-sets '$VALS' --band $BAND --width 1536 --depth 16 --heads 16 --param v --steps $CRITIC_STEPS --batch 1024 --micro-batch 128 --eval-every 500 --eval-n 256 --spot-exact-n 64 --spot-ode-steps 16 --max-hours 3.0" critic2
