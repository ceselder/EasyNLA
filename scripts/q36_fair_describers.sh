...
for V in v2 v1b; do
  ( for i in $(seq 1 400); do [ "$(nfiles q36/critic/$V 'ckpt_final.pt')" -ge 1 ] && break; [ $((i % 10)) -eq 0 ] && log "waiting for critic $V"; sleep 180; done; log "critic $V final exists"
    GT=H100 wait_gpu 1 || exit 1; run "--data-dir /vol/q36/data --ckpt /vol/q36/critic/$V/ckpt_final.pt --out /vol/q36/results/bits_${V}_describers.json --sets '$SAME' --n 300 --ode-steps 64 --skip-samples" bits_desc_$V
    GT=H100 wait_gpu 1 || exit 1; run "--data-dir /vol/q36/data --ckpt /vol/q36/critic/$V/ckpt_final.pt --out /vol/q36/results/bits_${V}_describersW.json --sets '$WITHW' --n 512 --ode-steps 64 --skip-samples" bits_descW_$V
