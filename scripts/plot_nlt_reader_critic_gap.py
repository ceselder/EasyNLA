...
SOURCES = [("teacher v1\npassage+lens+final", "reader_teacher_v1", "verdicts_union_pooled_null_teacher_v1_controls.json", "#2a78d6"),
           ...
           ("V0 verbalizer\nactivations only", "reader_v0_ao_tsv1", ("manual", 0.61, 0.10, "rl #169: V0 - z_dm, workspace band, text_union_v1n"), "#e34948")]
...
def main():
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12.5, 5.8), dpi=150, gridspec_kw={"wspace": 0.3}); x = np.arange(len(rows)); w = 0.38
    ...
    ax2.axhline(0, color=INK2, lw=0.8); ax2.set_ylim(0, max(4, max(r["content_bits"] + r["content_sem"] for r in rows) * 1.6)); ax2.set_ylabel("exact bits: log p(h_j | h_i, z) - log p(h_j | h_i, z_dm)")
    ...
    fig.tight_layout(rect=(0, 0.05, 1, 1)); os.makedirs(a.out_dir, exist_ok=True)
