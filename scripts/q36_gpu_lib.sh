...
wait_gpu(){ need=${1:-1}; cap=${2:-8}; exec 9>$LOCK; flock 9; for i in $(seq 1 900); do g=$(gpus_in_use); [ $((g + need)) -le $cap ] && { echo "[gpu] $(date -u +%H:%M) headroom: $g in use (cap $cap), launching $need"; return 0; }; [ $((i % 5)) -eq 0 ] && echo "[gpu] $(date -u +%H:%M) waiting: $g in use (cap $cap), need $need"; flock -u 9; sleep 120; flock 9; done; flock -u 9; return 1; }
