...
def main():
    ...
    ax1.set_ylabel("content bits = bits(z) − bits(z_dm), all bands"); ax1.set_ylim(0, ax1.get_ylim()[1] * 1.45); ax1.legend(frameon=False, loc="upper right", fontsize=10, ncol=1)
    ax1.set_title("Content: without a null term the adapter keeps 7–10 bits of paired content on lens text (the 8B encoder 11.5+, mostly by\nrecognising mismatches better); the null terms halve it by raising the wrong text (number above each bar = P(z beats z_dm); gate 0.75)", loc="left", fontsize=12.5)
    ax2.set_ylabel("random pair's text: bits vs the blind prior"); ax2.set_title("Random-pair offset (acceptance A3, one-sided since 23:13): a NEGATIVE value is calibration — a text about another pair makes h_j\nless likely — and passes; the null-term arms' +3–5-bit presence BONUS is what A3 rejects", loc="left", fontsize=12.5)
