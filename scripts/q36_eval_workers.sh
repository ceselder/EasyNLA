...
for i in $(seq 1 2000); do
  ...
  live=$(timeout 90 modal app list 2>/dev/null | grep nlt-q36 | grep -vE "stopped|stopping" | awk -F'│' '{gsub(/ /,"",$2); print $2}' | while read a; do grep -qE "^$a 1 evalq_w" $LOGD/gpu_ledger.txt && echo $a; done | wc -l)
