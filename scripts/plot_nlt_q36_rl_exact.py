...
def main():
    ...
    for f in sorted(glob.glob(f"{REP}/data/rl_{a.tag}_exact_*_*.json")):
        m = re.search(r"_exact_(frozen|other|judge2)_(\d+)\.json$", f)
        if not m: continue
    ...
    steps = sorted(rows)
    out = {"tag": a.tag, "steps": steps, "rows": {str(k): v for k, v in rows.items()}, "rule": "AMENDED 2026-09-25 16:12 UTC (orchestrator): the exact view under the claim-sensitive FROZEN judge decides - stop at >= 40 steps if its gain over step 0 is within one sem, regardless of the other judges; judge2 (critic v4 step 2000, the strongest large-set twins of the day) is the claim-sensitive second judge; 'other' (critic v2 step 3000) is a PLAUSIBILITY column only (no one-claim sensitivity on the large twin set: .46 / .54; prefers swapped bullets 2:1 on policy text)"}
    verdict = "pending"
    if steps and 0 in rows:
        ...
        if last >= a.min_steps and "frozen" in gains:
            fr = gains["frozen"]; j2 = gains.get("judge2")
            verdict = ("MOVES EXACT CONTENT under the frozen judge (gain > 1 sem)" + (f"; second judge (critic v4 step 2000) {'agrees' if j2['significant'] else 'does not confirm'}" if j2 else "")) if fr["significant"] else "STOP: FM-reward RL does not move exact content under the claim-sensitive frozen judge (gain within one sem at >= 40 steps)"
    ...
    def jname(j):
        ck = next((rows[s][j]["judge"] for s in steps if j in rows[s]), ""); import os as _o
        return {"frozen": "FROZEN reward judge", "judge2": "claim-sensitive 2nd judge", "other": "plausibility column (no one-claim sensitivity)"}[j] + f": {_o.path.basename(_o.path.dirname(ck))} {_o.path.basename(ck).replace('ckpt_', '').replace('.pt', '')}"
    fig, axes = plt.subplots(2, 2, figsize=(11, 8.6)); axes = axes.ravel(); axes[3].axis("off")
    for ax, j, title in ((axes[0], "frozen", "Decides the run"), (axes[1], "judge2", "Confirms"), (axes[2], "other", "Plausibility only")):
        xs = [s for s in steps if j in rows[s]]
        if not xs: ax.set_title(f"{title}: {j} (no evals yet)", fontsize=11); continue
        title = f"{title} - {jname(j)}\nexact Heun-32 content bits"
