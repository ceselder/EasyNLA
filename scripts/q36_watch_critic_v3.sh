...
json.dump(d, open('$D/critic_${TAG}_passes.json', 'w'))" 2>/dev/null
  new=0
  for f in $(timeout 120 modal volume ls nlt q36/results 2>/dev/null | grep -oE "bits_${TAG}_step[0-9]+(_train)?\.json" | sort -u); do
    ...
    case "$f" in *_train.json) log "pulled $f (train rows)"; continue;; esac
