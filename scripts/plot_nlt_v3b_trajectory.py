...
ARMS = [("fbp", "0.6B encoder, full pool, frozen POOLED prior", CAT[0], "-"), ("fbpc", "… the step-7000 checkpoint continued with a DECAYING learning rate", CAT[3], "--"), ("e2p", "Qwen3-8B encoder, full pool, frozen POOLED prior", CAT[6], "-"),
        ("e2sentp", "Qwen3-8B encoder, prose-only pool, frozen POOLED prior", CAT[2], "-"), ("e2sentpc", "… prose-only 8B arm, DECAYING rate (stopped)", CAT[5], "--"), ("fb", "0.6B encoder, full pool, frozen SQUASH prior (stopped)", CAT[7], ":")]
...
def main():
    ...
    for r, (st, slab) in enumerate(SETS):
        ...
        ax1.set_ylabel("exact bits (true text vs empty text)"); ax2.set_ylabel("content bits"); ax2.axhline(0, color=INK2, lw=0.9)
        for ax in (ax1, ax2):
