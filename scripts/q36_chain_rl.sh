...
TAG=${TAG:-v1}; RL_TAG=${RL_TAG:-rl_v1}; RL_GPUS=${RL_GPUS:-4}; STEPS=${STEPS:-150}; BATCH=${BATCH:-16}; GROUP=${GROUP:-8}; MAXG=${MAXG:-8}; BAND=${BAND:?set BAND}; KL=${KL:-0.02}; LAM=${LAM:-0.05}
...
wait_gpu(){ need=${1:-1}; for i in $(seq 1 600); do g=$(gpus_in_use); [ $((g + need)) -le $MAXG ] && { log "GPU headroom: $g in use, launching $need"; return 0; }; [ $((i % 5)) -eq 0 ] && log "waiting for GPU headroom ($g in use, need $need)"; sleep 120; done; return 1; }
