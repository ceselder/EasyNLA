...
# 1. the re-dump (app ap-rcaEn2liINeWOWN3GXtnVE) overwrites verbalizer_v1.parquet: wait for a mean text length above the capped dump's 497 chars
...
run hf 3 sft_verbalizer.py "--data-dir /vol/q36/data --text '$TX/train/craft_full__*.parquet' --val-text '$TX/val/craft_full__*.parquet' --out /vol/q36/verbalizer/v1b --band $BAND --steps 600 --batch 4 --grad-accum 8 --lr 3e-5 --max-len 256 --no-grad-ckpt --eval-every 100 --save-every 200 --wandb-name sft_v1b" sft_v1b 3 3
waitn q36/verbalizer/v1b "final" 1 || exit 1
GT=H100 wait_gpu 1 || exit 1
run hf 1 dump_verbalizer.py "--data-dir /vol/q36/data --adapter /vol/q36/verbalizer/v1b/final --pairs-text '$TX/val/craft_full__*.parquet' --n 1024 --band $BAND --max-new 256 --out /vol/q36/dumps/verbalizer_v1b.parquet" dump_v1b 1
