...
COL = ["#2a78d6", "#eb6834", "#1baf7a", "#eda100"]       # validated categorical slots, fixed order
INK = "#52514e"
...
out["benchmark_twins"] = own_wrong(ax[0, 0], "benchmark_twins", "(a) Benchmark twins: gain is not from\nthe activation")
last = TAGS[-1]; fams = ["internal", "text", "semantic"]; a = ax[0, 1]; x = np.arange(3); F = S[last]["synthetic_twins_by_family"]
...
a.set_ylabel("true claim above its twin (%)"); a.set_title("(b) Latest critic: templated claims are read,\nGemma twins mostly are not"); a.legend(fontsize=10, frameon=False, loc="upper right")
out["synthetic_twins_by_family_" + last] = {f: {"own": F[f]["own"], "wrong_activation": F[f]["wrong_activation"]} for f in fams}

Ns = ["16", "64", "256"]
for j, (d, title) in enumerate([("act_to_claim", "(c) Picking an activation's claim improves"), ("claim_to_act", "(d) Picking a claim's activation collapses")]):
    a = ax[1, j]
...
fig.suptitle("Training against false twins taught a claim-only shortcut: the latest critic's benchmark twin score (67%)\n"
             "barely drops when scored against the wrong activation (65%); same-template retrieval is the honest test", fontsize=13)
fig.tight_layout(rect=(0, 0, 1, 0.93))
for ext in ("png", "pdf"): fig.savefig(f"{REP}/controls.{ext}", bbox_inches="tight")
json.dump(out, open(f"{REP}/data/controls_plot.json", "w"), indent=1); print("wrote", f"{REP}/controls.png")
