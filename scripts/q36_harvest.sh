...
for i in $(seq 1 400); do
  raw=$(timeout 90 modal app list 2>/dev/null); [ -z "$raw" ] && { log "app list read failed (transient); skipping this round"; sleep 120; continue; }     # an empty list must never relaunch a live engine
