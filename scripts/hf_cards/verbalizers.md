...
Late additions (DECISIONS v1.33/v1.34, the three-way teacher-dossier test; identical V0 recipe, sentences only, one row per pair on the SAME pair ids): `v0c*` = V0c (teacher + feature dossier sentences), `v0d*` = V0d (dossier-only sentences), `*matched*` = the V0-matched control (teacher-sonnet-v1 sentences), when present; their 4096-pair val dumps are in the fixed-eval dataset under data/rl_dumps/.
