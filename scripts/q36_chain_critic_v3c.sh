...
(systemd-run --user --scope -q -p MemoryMax=1G --setenv=QUEUE_MODE=1 --setenv=PRIO=1 --setenv=TAG=v3c --setenv=MINSTEP=500 bash $LOGD/watch_critic_v3.sh >> $LOGD/watch_critic_v3c.out 2>&1 &)
