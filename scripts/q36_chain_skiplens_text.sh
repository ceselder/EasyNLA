...
run(){ timeout 900 modal run --detach scripts/modal_nlt_q36.py --task "$1" --gpus 1 --script "$2" --args "$3" 2>&1 | grep -E "SPAWNED|modal.com/apps|rror" | sed "s/^/[$4] /" | tee -a /home/celeste/nlt-q36-logs/apps.txt; }
