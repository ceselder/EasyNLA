...
SOURCES = [("reader_teacher_v1", "teacher\n+lens\n+final", "#2a78d6"), ("reader_teacher_nolens_v1", "teacher\npassage\nonly", "#1baf7a"), ("reader_lensdiff_jlens_L1", "J-lens\nsen-\ntence", "#4a3aa7"),
...
def main():
    ...
    fig, axes = plt.subplots(2, 2, figsize=(15, 11.5), dpi=150, gridspec_kw={"wspace": 0.16, "hspace": 0.5}); axes = axes.ravel()
    ...
    for ax, (task, title, chance) in zip(axes, TASKS):
        ...
        ax.set_xticks(x); ax.set_xticklabels([l for _, l, _ in srcs], fontsize=9.5, rotation=0, ha="center"); ax.set_ylim(0, 100); ax.set_ylabel("reader accuracy, %")
