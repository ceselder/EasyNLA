...
SOURCES = [("reader_teacher_v1", "teacher\n+lens\n+final", "#2a78d6"), ("reader_teacher_nolens_v1", "passage\nonly\nteacher", "#1baf7a"), ("reader_lensdiff_jlens_L1", "J-lens\nsen-\ntence", "#4a3aa7"),
           ("reader_v0_ao_tsv1", "verbal-\nizer", "#e34948"), ("reader_lensdiff_jlens_L3", "J-lens\nlists", "#e87ba4"), ("reader_prelim_v1n_40", "plain\nRL\nstep 40", "#008300"),
           ("reader_ref_v4_20", "RL\nnbr.\ndistr.", "#4a3aa7"), ("reader_dossier_sonnet_v1_v1", "dossier\n(no\npassage)", "#e87ba4"), ("reader_teacher_dossier_v1_v1", "teacher\n+ dos-\nsier", "#e34948"),
           ("reader_v0-matched", "SFT:\nteacher", "#87867F"), ("reader_v0c-tdv1", "SFT:\nteacher\n+dossier", "#eda100"), ("reader_v0d-dos", "SFT:\ndossier", "#1baf7a")]
...
def main():
    ...
    fig, axes = plt.subplots(2, 2, figsize=(16.5, 12), dpi=150, gridspec_kw={"wspace": 0.16, "hspace": 0.5}); axes = axes.ravel()
    ...
    for ax, (task, title, chance) in zip(axes, TASKS):
        ...
        ax.set_xticks(x); ax.set_xticklabels([l for _, l, _ in srcs], fontsize=9, rotation=0, ha="center"); ax.set_ylim(0, 100); ax.set_ylabel("reader accuracy, %")
