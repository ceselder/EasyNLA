...
SOURCES = [("reader_teacher_v1", "teacher\n+lens +final", "#2a78d6"), ("reader_teacher_nofinal_v1", "teacher\n+lens", "#eb6834"),
           ...
           ("reader_prelim_v1n_20", "plain-PMI RL\nstep 20", "#eda100"), ("reader_prelim_v1n_40", "plain-PMI RL\nstep 40", "#008300"), ("reader_ref_v1_0", "list-naming\nwarm start for RL", "#87867F"), ("reader_ref_v4_20", "RL, neighbour-position\ndistractors, step 20", "#4a3aa7"), ("reader_dossier_sonnet_v0_v1", "feature dossier v0\n(no passage)", "#b07aa1"), ("reader_dossier_sonnet_v1_v1", "feature dossier v1\n(no passage)", "#e87ba4"), ("reader_teacher_dossier_v1_v1", "teacher + dossier\n(passage + features)", "#e34948"), ("reader_v0_matched_0", "verbalizer SFT on\nteacher (matched\ncontrol)", "#87867F"), ("reader_v0-matched", "verbalizer SFT on\nteacher (matched\ncontrol)", "#87867F"), ("reader_v0c_td_0", "verbalizer SFT on\nteacher + dossier", "#eda100"), ("reader_v0c-tdv1", "verbalizer SFT on\nteacher + dossier", "#eda100"), ("reader_v0d_dos_0", "verbalizer SFT on\ndossier only", "#1baf7a"), ("reader_v0d-dos", "verbalizer SFT on\ndossier only", "#1baf7a")]
...
def main():
    ...
    for ax, (task, title, chance) in zip(axes, TASKS):
        ...
        ax.set_xticks(x); ax.set_xticklabels([l for _, l, _ in srcs], fontsize=9, rotation=38, ha="right", rotation_mode="anchor"); ax.set_ylim(0, 100); ax.set_ylabel("reader accuracy, %")
    ...
    fig.suptitle("\n".join(textwrap.wrap("A verbalizer trained on activations alone writes sentences from which a reader recovers the model's next token and document position far above chance; the referential RL arm with a neighbour distractor (arm E, step 20) is the only RL checkpoint that reads better than its warm start; feature-dossier descriptions read like the lens sentence, and a describer shown both the passage and the dossier reads like the teacher — the passage sets what a reader can recover", 120)), fontsize=12.5, x=0.01, ha="left")
