...
RL_TAG=${RL_TAG:-rl_v5}; FROZEN=${FROZEN:-/vol/q36/critic/v5/ckpt_step000500.pt}; OTHER=${OTHER:-/vol/q36/critic/v2/ckpt_step3000.pt}; JUDGE2=${JUDGE2:-/vol/q36/critic/v4/ckpt_step002000.pt}; EVERY=${EVERY:-20}; SFT=${SFT:-/vol/q36/verbalizer/v1b/final}
...
declare -A Q; miss=0
queued(){ grep -q "^\[$1\] QUEUED" $LOGD/evalq.txt 2>/dev/null; }     # idempotent across restarts (15:03: two restarts re-enqueued the step-0 judge evals -> 6 specs for 2 evals)
for it in $(seq 1 600); do
  ...
  for st in $steps; do stn=$((10#$st)); [ $((stn % EVERY)) -eq 0 ] || continue
    ...
    if ! echo "$RLS" | grep -q "dumpsL_$st.parquet"; then
      [ -n "${Q[dump_$st]:-}" ] || queued ptw_dump_${RL_TAG}_$st || { enqueue_eval "--data-dir /vol/q36/data --adapter $AD --out $RLD/dumpsL_$st.parquet --split val --pair-ids-file $PAIRS --n 1024 --batch 64 --max-new 208 --source ${RL_TAG}_step$st" ptw_dump_${RL_TAG}_$st 1 dump_verbalizer.py && Q[dump_$st]=1; }; continue; fi
    if ! echo "$RLS" | grep -q "twinsL_$st.parquet"; then
      [ -n "${Q[tw_$st]:-}" ] || queued ptw_build_${RL_TAG}_$st || { enqueue_eval "--dump $RLD/dumpsL_$st.parquet --out $RLD/twinsL_$st.parquet" ptw_build_${RL_TAG}_$st 1 policy_twins.py && Q[tw_$st]=1; }; continue; fi
    for J in frozen other judge2; do out=rl_${RL_TAG}_ptwinsL_${J}_$st; [ -f $D/$out.json ] && continue
...
print(' | '.join(f\"{k}: P {x['p_true_gt_twin']:.3f} [{x['ci95_p'][0]:.3f},{x['ci95_p'][1]:.3f}] FM {x['proxy_p_true_gt_twin']:.3f} [{x['proxy_ci95_p'][0]:.3f},{x['proxy_ci95_p'][1]:.3f}] n {x['n_positions']}\" for k, x in v.items() if k in ('twin_shift', 'twin_new', 'dm_full')))" 2>/dev/null)"; systemd-run --user --scope -q -p MemoryMax=2G python3 scripts/plot_nlt_q36_policy_twins.py --tag $RL_TAG 2>&1 | grep -E "^saved|Traceback" | head -2; (cd /home/celeste/shared/reports/nlt-27b-olens && systemd-run --user --scope -q -p MemoryMax=1G python3 build_html.py >/dev/null 2>&1); }; continue; fi
      if [ -n "${Q[$out]:-}" ] || queued $out; then Q[$out]=1; continue; fi; CK=$FROZEN; [ $J = other ] && CK=$OTHER; [ $J = judge2 ] && CK=$JUDGE2
