...
PRIOR=/vol/critic/none_v1_squash/ckpt_final.pt
...
modal run scripts/modal_nlt_critic.py --task train --tag $TAG --data $D --extra "--cond text --init-from $PRIOR --freeze-prior 1 --src-rms 0 --squash 1.0 --resume $RESUME --text-parquet $TRAIN --val-text-parquet $VAL --steps ${STEPS:-20000} --batch 512 --lr 1e-4 --lr-decay none --warmup 300 --eval-every 500 --eval-n 2048 --save-every 500 --keep-every 500 --d-model 3072 --d-mlp 12288 --n-layers 12 --n-slots 16 --n-heads 8 --d-head 64 --gate-rank 256 --p-uncond 0.3 --enc-max-len 256 --data-device cpu --enc-model Qwen/Qwen3-8B --enc-layer 24 --max-hours ${MAX_HOURS:-3.5}" > $LOG/critic_$TAG.log 2>&1
