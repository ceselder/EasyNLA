...
for i in $(seq 1 600); do n=$(timeout 120 modal volume ls nlt q36/results 2>/dev/null | grep -cE "bits_v2_(sources|loo|singles)\.json"); [ $((i % 5)) -eq 0 ] && log "bits_v2 files: $n/3"; [ "$n" -ge 3 ] && break; sleep 120; done
for f in sources loo singles; do timeout 300 modal volume get nlt q36/results/bits_v2_$f.json $D/bits_v2_$f.json --force >/dev/null 2>&1 && log "pulled bits_v2_$f.json"; done
