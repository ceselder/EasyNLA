...
def main():
    ...
    # manifest
    m = controls.build_manifest(pairs, zs["clean"], PrefixStore.from_infra(meta, docs)); assert set(m.variant) == {"orig", "empty", "dm", "rp", "copy", "wrong_j", "wrong_i", "shuf_words", "dm_doc"}, set(m.variant)
    ...
    rng = np.random.default_rng(0); base = {"orig": 20, "dm": 5, "rp": 0, "copy": 1, "wrong_j": -10, "wrong_i": 8, "empty": 0, "shuf_words": 6, "dm_doc": 12}
