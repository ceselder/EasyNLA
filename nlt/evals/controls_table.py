...
def main():
    ...
    for f in sorted(glob.glob(os.path.join(a.dir, f"verdicts_{a.critic}_*_controls.json"))):
        ...
        r = {"source": src, "n": s["orig"]["n"], "orig": g("orig"), "orig_ci": s["orig"]["ci95"], "dm": g("dm"), "rp": g("rp"), "shuf_words": g("shuf_words"), "copy": g("copy"), "src_desc": g("src_desc"), "wrong_j": g("wrong_j"),
             ...
             "p_orig_gt_null": (1 - s["orig"]["share_nonpositive"]) if s["orig"].get("share_nonpositive") is not None else None,      # v1.16: P(z > null) next to content
    ...
    md = [f"### {a.critic}: controls on the fixed eval set (exact bits vs the blind prior; content = orig − dm)", "",
          "| source | n | orig | dm | rp | shuf | copy | src | wrong_j | content | form | depthG | P(>null) | P(>dm) | P(>rp) | P(>shuf) | bits/tok | content pre/ws/motor |", "|" + "---|" * 18]
    for r in rows:
        cb = r["content_by_band"]; md.append(f"| {r['source']} | {r['n']} | {f(r['orig'])} | {f(r['dm'])} | {f(r['rp'])} | {f(r['shuf_words'])} | {f(r['copy'],1)} | {f(r['src_desc'])} | {f(r['wrong_j'],0)} | {f(r['content'])} | {f(r['form'],1)} | {f(r['depth_generic'])} | {f(r['p_orig_gt_null'])} | {f(r['p_orig_gt_dm'])} | {f(r['p_orig_gt_rp'])} | {f(r['p_orig_gt_shuf'])} | {f(r['bits_per_token'],3)} | {f(cb.get('pre'))}/{f(cb.get('workspace'))}/{f(cb.get('motor'))} |")
