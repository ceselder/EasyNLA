...
def main():
    ...
    for s in range(0, N, a.batch):
        ...
        slot = pack_slot(vecs, [sp.positions for sp in specs], offsets=offs, scale_list=[mid_scales(v) for v in vecs] if a.mid_scale == "relmax" else None); n_expected += int((slot[1] >= 0).sum())
        for g in range(a.n_samples):
            inj.ref[0] = tuple(t.to(dev) for t in slot); inj.reset_count()
            try:
                with torch.no_grad():
    ...
    summ = {"rows": len(rows), "pairs": N, "n_samples": a.n_samples, "temperature": a.temperature, "init": a.init, "source": a.source, "path_mode": a.path_mode, "mid_scale": a.mid_scale,
