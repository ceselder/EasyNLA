...
Specs (--specs, ';'-separated) name columns of the --data parquet(s): 'h_L24' = that layer's activation, 'h_L42-h_L24' = the difference
(later minus earlier). Vectors are unit-normalised (norm-matched injection makes the scale irrelevant). One parquet per spec in --out-dir:
  row int32 (index into the data rows), sample int32 (0 = greedy, 1..n sampled), spec string, ids list<int32>, text string
...
ap.add_argument("--data", required=True, help="parquet or glob with the activation columns"); ap.add_argument("--out-dir", required=True)
...
ap.add_argument("--specs", required=True, help="';'-separated: h_L24 | h_L42-h_L24 | any column name")
...
ap.add_argument("--pairs", default=None, help="PAIR MODE: pairs parquet (pair_id, shard, row, i, j); rows = this shard's pairs; specs may be v_i / v_j / v_delta (built from the acts parquet given as --data)")
...
def main():
    ...
    from common import D_MODEL, MARKER_ID, build_av_prompt, load_tokenizer, olens_prompt
    ...
    specs = [s.strip() for s in args.specs.split(";") if s.strip()]; os.makedirs(args.out_dir, exist_ok=True)
    def spec_name(s): return s.replace("-", "_minus_")
    todo = [s for s in specs if not os.path.exists(os.path.join(args.out_dir, spec_name(s) + ".parquet"))]
    print(f"[rollout] {len(todo)}/{len(specs)} specs to do: {todo}", flush=True)
    if not todo: print("ROLLOUT_DONE (nothing to do)", flush=True); return
    pair_ids = None
    if args.pairs:                                                   # PAIR MODE: one (i, j) per row of this shard's pair list; v_i / v_j / v_delta from the acts parquet
        import pandas as pd
        P = pq.read_table(args.pairs).to_pandas(); P = P[P["shard"] == args.shard_index].reset_index(drop=True)
        if args.n_rows: P = P.iloc[args.skip_rows: args.skip_rows + args.n_rows].reset_index(drop=True)
        assert len(files) == 1, "pair mode takes ONE acts parquet"; n = len(P); pair_ids = P["pair_id"].tolist()
        layers = sorted(set(P["i"].tolist()) | set(P["j"].tolist())); tb = pq.read_table(files[0], columns=[f"h_L{L}" for L in layers] + ["row"])
        rowpos = {int(r): k for k, r in enumerate(tb.column("row").to_numpy())}; ridx = np.array([rowpos[int(r)] for r in P["row"]])
        ...
        COL = {"v_i": vi, "v_j": vj, "v_delta": vj - vi}; print(f"[rollout] PAIR MODE shard {args.shard_index}: {n} pairs, layers {layers}", flush=True)
    else:
        need_cols = sorted({c for s in todo for c in s.split("-")})
        tbs = [pq.ParquetFile(f).read(columns=need_cols) for f in files]; tb = pa.concat_tables(tbs).slice(args.skip_rows, args.n_rows if args.n_rows else None); n = tb.num_rows
        COL = {c: torch.tensor(fsl(tb, c, D_MODEL, np.float32)) for c in need_cols}; print(f"[rollout] {n} rows, columns {need_cols}", flush=True)
    ...
    ban = {}
    if args.prompt == "av": ban = {int(tok.eos_token_id): -100.0}          # the AV prompt writes a continuation: never stop early (bullets: EOS ends the list)
    total = 0
    for si, s in enumerate(todo):
        ...
        for i in range(n):
            ...
            if args.n_samples > 0:
                ...
                q += 1
        cols = {"row": pa.array(rows, pa.int32()), "sample": pa.array(samples, pa.int32()), "spec": pa.array([s] * len(rows), pa.string()),
                "ids": pa.array(ids_col, pa.list_(pa.int32())), "text": pa.array(texts, pa.string())}
        if pair_ids is not None: cols["pair_id"] = pa.array([pair_ids[r - args.skip_rows] for r in rows], pa.string())
        table = pa.table(cols)
        outp = os.path.join(args.out_dir, spec_name(s) + ".parquet"); pq.write_table(table, outp + ".tmp", compression="zstd"); os.replace(outp + ".tmp", outp); total += len(rows)
        ntok = float(np.mean([len(x) for x in ids_col]))
        print(f"[rollout] {si + 1}/{len(todo)} {s}: {n} rows -> {len(rows)} readouts in {t2 - t1:.1f}s ({len(rows) / max(t2 - t1, 1e-9):.1f} readouts/s, {ntok:.0f} tok each) | total {total} | {(time.time() - t0) / 60:.1f} min", flush=True)
        for i in range(2): print(f"  row {i} greedy: {texts[i * (1 + args.n_samples)]!r}", flush=True)
    try:
