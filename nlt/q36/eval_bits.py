...
def main():
    ...
    if a.fixed_from_twins:
        ...
        if a.pair_ids_file:
            ids = set(l.strip() for f_ in a.pair_ids_file.split(",") for l in open(f_) if l.strip()); n0 = len(vp); vp = vp[vp["pair_id"].isin(ids)]
            print(f"[bits] pair-ids file {a.pair_ids_file}: {len(vp)} of {n0} rows are in the trained slice ({len(ids)} ids)", flush=True)
        vp = vp.drop_duplicates("pair_id")[["pair_id", "pos_idx", "i", "j"]]
    ...
    for label, tm in sets.items():
        ...
        res = {"n": n, "n_common": sum(1 for k in idx if k in set(common)), "pair_ids": [pid_all[k] for k in idx], "n_tokens_mean": ntok,
               ...
               "neighbours": NBRES, "cos_condmean": {k: float(COS[k].mean()) for k in COS}, "cos_samples": {k: float(CS[k].mean()) for k in CS}, "cos_sample_mean": {k: float(CSM[k].mean()) for k in CSM}, "p_cos_c_gt_u": float((COS["c"] > COS["u"]).float().mean()),
